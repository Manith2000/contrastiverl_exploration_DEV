...
flags.DEFINE_bool(
    "add_uid", False, "Whether to add a unique id to the log directory name"
)
flags.DEFINE_string(
    "alg",
    "contrastive_cpc",
    "Algorithm type, e.g. default is contrastive_cpc with no entropy or KL losses",
)
...
def get_env(
    ...
    return contrastive_utils.make_environment(
        env_name, start_index, end_index, seed=seed, fixed_start_end=fixed_start_end
    )
...
def get_program(params):
    ...
    environment, obs_dim = get_env(
        env_name, config.start_index, config.end_index, seed, fix_goals=fix_goals
    )
    ...
    env_factory_no_extra_fixed_goals = lambda seed: env_factory_fixed_goals(seed)[
        0
    ]  # Remove obs_dim.
